...
if (!function_exists('get_cached_products')) {
    function get_cached_products($category_id = null)
    {
        return Cache::remember('products-category-' . $category_id, 86400, function () use ($category_id) {
            return Product::where('category_id', $category_id)->latest()->take(5)->get();
        });
    }
}
...
if (!function_exists('home_discounted_price')) {
    function home_discounted_price($product, $formatted = true)
    {
        ...
        $str = '';
        if(!empty($product->specification_options)) {
            foreach (json_decode($product->specification_options) as $key => $specification) {
                foreach($specification->values as $key1 => $spec_val) {
                    if ($str != null) {
                        $str .= '-' . str_replace(' ', '', $spec_val);
                    } else {
                        $str .= str_replace(' ', '', $spec_val);
                    }

                    $res = $product->specification->where('variant', $str)->first();
                    if(!empty($res)) {
...
function hex2rgba($color, $opacity = false) {
    ...
    // Check if it's a 3-character hex and convert it to 6-character hex
    if (strlen($color) == 3) {
    ...
    // If opacity is not specified, default to 1
    if ($opacity === false) {
        $opacity = 1;
    }
...
if (!function_exists('get_products_count')) {
    function get_products_count($user_id = null)
    {
        ...
        return $products_query->isPublished()->count();
    }
}
...
if (!function_exists('get_product_min_unit_price')) {
    function get_product_min_unit_price($user_id = null)
    {
        ...
        return $product_query->isPublished()->min('unit_price');
    }
}
...
if (!function_exists('get_product_max_unit_price')) {
    function get_product_max_unit_price($user_id = null)
    {
        ...
        return $product_query->isPublished()->max('unit_price');
    }
}
...
if (!function_exists('get_featured_products')) {
    function get_featured_products()
    {
        return Cache::remember('featured_products', 3600, function () {
            $product_query = Product::query();
            return $product_query->where('featured', '1')->latest()->limit(12)->get();
        });
    }
}
...
if (!function_exists('get_seller_products')) {
    ...
    {
        $product_query = Product::query();
        return $product_query->where('user_id', $user_id)->isPublished()->orderBy('created_at', 'desc')->limit(15)->get();
    }
}
...
if (!function_exists('get_shop_best_selling_products')) {
    ...
    {
        $product_query = Product::query();
        return $product_query->where('user_id', $user_id)->isPublished()->orderBy('num_of_sale', 'desc')->paginate(24);
    }
}
...
if (!function_exists('get_frequently_bought_products')) {
    function get_frequently_bought_products($product)
    {
        ...
        if($productSelectionType == 'product'){
            ...
            $fqbProducts = Product::whereIn('id', $fqbProductIds)->get();
        }
        elseif($productSelectionType == 'category'){
            ...
            if($fqbCategoryID != null){
                ...
                $fqbProducts = $fqbProducts->orderByRaw('RAND()')->take(10)->get();
            }
        }
        return $fqbProducts;
    }
...
if (!function_exists('get_brands_by_products')) {
    ...
    {
        $product_query = Product::query();
        $brand_ids =  $product_query->where('user_id', $usrt_id)->isPublished()->whereNotNull('brand_id')->pluck('brand_id')->toArray();
...
if (!function_exists('get_level_zero_categories')) {
    function get_level_zero_categories()
    {
        ...
        return $categories_query->orderBy('order_level', 'desc')->get();
    }
}
...
if (!function_exists('get_categories_by_products')) {
    ...
    {
        $product_query = Product::query();
        $category_ids = $product_query->where('user_id', $user_id)->isPublished()->pluck('category_id')->toArray();
